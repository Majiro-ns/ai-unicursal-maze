...
def _border_cells_by_side(grid_rows: int, grid_cols: int) -> Dict[str, List[int]]:
    """
    4辺ごとのセルIDリストを返す。

    コーナーは top / bottom に含める（left / right は中間行のみ）。
    → 全セルに重複なくいずれか1辺へ所属。
    ...
    top    = list(range(grid_cols))                               # row 0、全列
    bottom = list(range(num_cells - grid_cols, num_cells))        # row rows-1、全列
    left   = [grid_cols * r for r in range(1, grid_rows - 1)]    # col 0、中間行
    right  = [grid_cols * r + grid_cols - 1 for r in range(1, grid_rows - 1)]  # col cols-1
...
def find_image_guided_path(
    ...
    アルゴリズム:
      ...
      2. 入口: 4辺全セルの中で最も暗いセル（MAZE-Q3: 4隅限定を解除）
         出口: 入口の反対辺の中で最も明るいセル
      3. Dijkstra で入口→出口の最小コスト経路を求める

    Args:
        adj: 隣接リスト（post_process_density 後のループ含むグラフ）
        num_cells: グリッドのセル総数
    ...
    # --- 入口/出口の選択: 4辺全セルから暗部入口 → 反対辺の明部出口 ---
    # MAZE-Q3: 4隅限定を解除し、辺中央の暗部も入口候補とする
    sides = _border_cells_by_side(grid_rows, grid_cols)

    entrance = 0
    entrance_side = "top"  # デフォルト（sides に必ず top/bottom は存在する）
    best_lum = float("inf")
    for side_name, cells in sides.items():
        for c in cells:
            ...
            if lum < best_lum:
                best_lum = lum
                entrance = c
                entrance_side = side_name

    # 出口: 反対辺の中で最も明るいセル
    opp_side = _OPPOSITE_SIDE[entrance_side]
    exit_cell = max(sides[opp_side], key=lambda c: float(flat_lum[c]))
